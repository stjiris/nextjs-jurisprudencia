...
function TextQueryChip({ value }: { value: string }) {
    const id = `q-chip-${encodeURIComponent(value)}`;
    const searchParams = useSearchParams();
    const router = useRouter();
    const [editing, setEditing] = useState(false);
    ...
    const save = () => {
        ...
        if (trimmed && trimmed !== value) {
            router.push(`?${replaceSearchParams(searchParams, "q", trimmed, value)}`);
        }
        setEditing(false);
    };
    ...
    return (
        <div className="p-1 m-0 d-flex align-items-center" style={{background: "var(--secondary-gold)", borderBottom: "1px solid var(--primary-gold)"}}>
            <input type="checkbox" name="q" value={editing ? editValue.trim() || value : value} id={id} hidden defaultChecked={true}/>
            {editing
                ? <input
                    ref={inputRef}
                    autoFocus
...
export function TextQueryList({ qValues }: { qValues: string[] }) {
    return (
        <div className="d-flex flex-column my-1 border pb-1 flex-grow-1">
            <input
                ...
                autoComplete="off"
            />
            {qValues.map(val => <TextQueryChip key={val} value={val}/>)}
        </div>
    );
}
...
function FilterList({filtersUsed, accessKey, dontSuggest, showKey}: {filtersUsed: Record<string, string[]>, accessKey: keyof JurisprudenciaDocument | string, dontSuggest?: boolean, showKey?: string}){
    ...
    return <div className="d-flex flex-column my-1 border pb-1 flex-grow-1">
        ...
        <input type="text" className="form-control form-control-sm border-0 border-bottom rounded-0" name={accessKey} autoComplete="off" list={datalistId} placeholder={showKey || accessKey} onFocus={handleFocus}/>
